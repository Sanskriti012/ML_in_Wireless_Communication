...
%% in this question first we do all steps of last question then calculate NMSE and plot thenormalized mean square error(NMSE):
% Set hyperparameters
...
% Compute the normalized mean squared error (NMSE)
    NMSE(j) = norm(w_hat - w)^2/norm(w)^2;
end
% Plot the NMSE vs SNR
figure
plot(vr_dB, NMSE, 'o-')
xlabel('SNR (dB)')
ylabel('NMSE')
title('SBL for Sparse Regression')